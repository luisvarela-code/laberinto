...
class COLOR(Enum):
    '''
    Esta clase enumera los colores predefinidos para usar en la interfaz gráfica.
    Cada objeto COLOR tiene dos valores de color: el primero es para el tema oscuro
    y el segundo para el tema claro. Los primeros dos objetos son para el tema,
    y los siguientes colores son para los agentes en el laberinto.
    El primer valor es el color del agente y el segundo es el color de su huella.
    '''
...
class agent:
    '''
    Los agentes pueden ser colocados en el laberinto.
    Pueden representar un objeto virtual simplemente para indicar la celda seleccionada en el laberinto.
    ...
    def __init__(self,parentMaze,x=None,y=None,shape='square',goal=None,filled=False,footprints=False,color:COLOR=COLOR.blue):
        '''
    parentMaze --> El laberinto en el que se coloca el agente.
    x, y --> Posición del agente, es decir, la celda dentro de la cual se colocará el agente.
            El valor predeterminado es la esquina inferior derecha del laberinto.
        shape --> Cuadrado o flecha (como string).
        goal --> Valor predeterminado es el objetivo del laberinto.
        filled --> Para forma cuadrada, filled=False es un cuadrado más pequeño.
        Mientras que filled=True es un cuadrado más grande lleno en la celda completa.
        Esta opción no importa para la forma de flecha.
        footprints --> Cuando el agente se mueva a otra celda, sus huellas en la celda anterior pueden colocarse haciendo que esto sea Verdadero.
        color --> Color del agente.
        
        _orient --> No es necesario pasar esto.
                    Se usa con el agente de forma de flecha para mostrarlo girando.
        position --> No es necesario pasar esto.
        Es la celda (x, y).
        _head --> No es necesario pasar esto.
                    Es realmente el agente.
        _body --> No es necesario pasar esto.
                    Rastrea el cuerpo del agente (las posiciones anteriores de él).
    '''
    ...
    def _RCCW(self):
        '''
    Gira el agente en sentido antihorario (contrario a las manecillas del reloj).
    '''
    ...
    def _RCW(self):
        '''
    Gira el agente en sentido horario (en el sentido de las manecillas del reloj).
    '''
        def pointNew(p,newOrigin):
            return (p[0]-newOrigin[0],p[1]-newOrigin[1])
        w=self._parentMaze._cell_width
...
class textLabel:
    '''
    Clase para representar un agente en el laberinto.
    '''
...
class maze:
    '''
    Clase principal para crear un laberinto.
    '''
    def __init__(self,rows=10,cols=10):
        '''
        rows--> Número de filas del laberinto
        cols--> Número de columnas del laberinto
        Solo es necesario pasar estos dos argumentos. El resto se asignará automáticamente.
        maze_map--> Se establecerá como un diccionario. Las claves serán las celdas y
                    los valores serán otro diccionario con claves ['E', 'W', 'N', 'S'] para
                    Este, Oeste, Norte y Sur respectivamente, y los valores serán 0 o 1. 0 significa que 
                    la dirección (EWNS) está bloqueada. 1 significa que la dirección está abierta.
        grid--> Una lista de todas las celdas
        path--> El camino más corto desde el inicio (esquina inferior derecha) hasta la meta (por defecto esquina superior izquierda)
                Será un diccionario
        _win,_cell_width,_canvas -->    _win y _canvas son para la ventana Tkinter y el lienzo
                                        _cell_width es el ancho de la celda calculado automáticamente
        _agents-->  Una lista de agentes en el laberinto
        markedCells-->  Se utilizará para marcar algunas celdas particulares durante
                        el rastreo del camino por el agente
    '''
        self.rows=rows
        self.cols=cols
        self.maze_map={}
    ...
    def _Open_East(self,x, y):
        '''
    Método para abrir el paso hacia el Este en una celda específica.
    '''
    ...
    def CreateMaze(self,x=1,y=1,pattern=None,loopPercent=0,saveMaze=False,loadMaze=None,theme:COLOR=COLOR.dark):
        '''
    Método para crear un laberinto aleatorio.
    Args:
        x (int): Coordenada x de la posición inicial del laberinto.
        y (int): Coordenada y de la posición inicial del laberinto.
        pattern (str): Patrón visual del laberinto ('v' para vertical o 'h' para horizontal).
        loopPercent (int): Porcentaje de bucles en el laberinto.
        saveMaze (bool): Indica si se debe guardar el laberinto generado como un archivo CSV.
        loadMaze (str): Ruta del archivo CSV que contiene un laberinto predefinido.
        theme (COLOR): Tema del laberinto (dark o light).
    '''
        _stack=[]
        _closed=[]
        self.theme=theme
        ...
        def BFS(cell):
            ...
            Esto se usará solo cuando haya múltiples rutas (loopPercent>0) o
            Maze se carga desde un archivo CSV.
            Si se genera un laberinto perfecto y sin el archivo de carga, este método
            no se utilizará ya que la generación del Laberinto calculará el camino.
            '''
            frontier = deque()
            ...
            while cell!=(self.rows,self.cols):
                try:
                    ...
                    print('¡Camino a la meta no encontrado!')
                    return
            return fwdPath
    ...
    def _drawMaze(self,theme):
        ...
        self._win.title('PYTHON MAZE WORLD de Luis Alberto')
        ...
        k=3.25
        if self.rows>=99 and self.cols>=99:
            k=0
        elif self.rows>=90 and self.cols>=80:
            k=1
        elif self.rows>=80 and self.cols>=70:
            k=1.5
